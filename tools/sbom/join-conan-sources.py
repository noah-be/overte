...
def main():
    parser = argparse.ArgumentParser(description='Offline completed package/source-ledger join, not payload or license acceptance')
    for name in ('source-closure', 'actual-graph', 'expected-graph', 'checkpoint', 'expected-source-sha',
    ...
    try:
        result = verify_sources(args.source_closure, args.actual_graph, args.expected_graph, args.checkpoint,
        ...
        print('SOURCE_JOIN_INPUT_INVALID', file=sys.stderr); return 1
