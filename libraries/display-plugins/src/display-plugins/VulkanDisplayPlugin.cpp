...
public:
    ...
    void setNewDisplayPlugin(VulkanDisplayPlugin* plugin) {
        Lock lock(_mutex);
        if (isRunning()) {
...
bool VulkanDisplayPlugin::startStandBySession() {
...
void VulkanDisplayPlugin::endSession() {
    deactivateSession();
    Parent::endSession();
}
...
void VulkanDisplayPlugin::uncustomizeContext() {
...
bool VulkanDisplayPlugin::eventFilter(QObject* receiver, QEvent* event) {
    switch (event->type()) {
...
void VulkanDisplayPlugin::captureFrame(const std::string& filename) const {
    ...
    withOtherThreadContext([&] {
        ...
        TextureCapturer captureLambda = [&](const gpu::TexturePointer& texture)->storage::StoragePointer {
            return textureToKtxVulkan(*texture);
        };
...
void VulkanDisplayPlugin::updateFrameData() {
...
std::function<void(gpu::Batch&, const gpu::TexturePointer&)> VulkanDisplayPlugin::getHUDOperator() {
...
void VulkanDisplayPlugin::compositePointer() {
    auto& cursorManager = Cursor::Manager::instance();
...
void VulkanDisplayPlugin::setupCompositeScenePipeline(gpu::Batch& batch) {
    batch.setPipeline(_drawTexturePipeline);
}

void VulkanDisplayPlugin::compositeScene() {
    render([&](gpu::Batch& batch) {
        batch.enableStereo(false);
        batch.setFramebuffer(_compositeFramebuffer);
...
void VulkanDisplayPlugin::compositeLayers() {
    updateCompositeFramebuffer();
...
void VulkanDisplayPlugin::internalPresent() {
    render([&](gpu::Batch& batch) {
...
float VulkanDisplayPlugin::droppedFrameRate() const {
    return _droppedFrameRate.rate();
}

float VulkanDisplayPlugin::presentRate() const {
...
void VulkanDisplayPlugin::resetPresentRate() {
    // FIXME
    // _presentRate = RateCounter<100>();
}

float VulkanDisplayPlugin::renderRate() const {
...
QImage VulkanDisplayPlugin::getSecondaryCameraScreenshot() {
    auto textureCache = DependencyManager::get<TextureCache>();
...
glm::uvec2 VulkanDisplayPlugin::getSurfacePixels() const {
    uvec2 result;
...
glm::uvec2 VulkanDisplayPlugin::getSurfaceSize() const {
    uvec2 result;
    auto window = _container->getPrimaryWidget();
    if (window) {
...
void VulkanDisplayPlugin::assertNotPresentThread() const {
    Q_ASSERT(QThread::currentThread() != _presentThread);
}

void VulkanDisplayPlugin::assertIsPresentThread() const {
    Q_ASSERT(QThread::currentThread() == _presentThread);
}

bool VulkanDisplayPlugin::beginFrameRender(uint32_t frameIndex) {
    withNonPresentThreadLock([&] {
...
const gpu::BackendPointer& VulkanDisplayPlugin::getBackend() const {
...
VulkanDisplayPlugin::~VulkanDisplayPlugin() {
}
