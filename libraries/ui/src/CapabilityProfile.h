...
inline bool preferenceAllowed(Product product, const std::string& category, const std::string& name) {
    ...
    if (category == "Touch Camera Sensitivity") { return product == Product::Phone || product == Product::IOS; }
    if (category == "Mouse Sensitivity" && (product == Product::Phone || product == Product::IOS)) { return false; }
