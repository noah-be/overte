...
static ScriptValue debugPrint(ScriptContext* context, ScriptEngine* engine) {
    ...
    if (!scriptManager) {
        qCDebug(scriptengine_script) << overte::security::diagnosticEvent(overte::security::DiagnosticEvent::Redacted);
        return ScriptValue();
    }
    ...
    if (loggerInterface && loggerInterface->showSourceDebugging()) {
        ...
        scriptManager->print(message);
        // Public logs exclude script identity and contents; the internal print route is unchanged.
        qCDebug(scriptengine_script) << overte::security::diagnosticEvent(overte::security::DiagnosticEvent::Redacted);
    }
