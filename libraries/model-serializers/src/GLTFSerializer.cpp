...
static Sampler::Filter filterFromGL(cgltf_int minFilter, cgltf_int magFilter) {
    if (magFilter == GL_NEAREST) {
        if (minFilter == GL_NEAREST) {
            return Sampler::Filter::FILTER_MIN_MAG_POINT;
        } else if (minFilter == GL_LINEAR) {
            return Sampler::Filter::FILTER_MIN_LINEAR_MAG_POINT;
        } else if (minFilter == GL_NEAREST_MIPMAP_NEAREST) {
            return Sampler::Filter::FILTER_MIN_MAG_MIP_POINT;
        } else if (minFilter == GL_NEAREST_MIPMAP_LINEAR) {
            return Sampler::Filter::FILTER_MIN_MAG_POINT_MIP_LINEAR;
        } else if (minFilter == GL_LINEAR_MIPMAP_NEAREST) {
            return Sampler::Filter::FILTER_MIN_LINEAR_MAG_MIP_POINT;
        } else if (minFilter == GL_LINEAR_MIPMAP_LINEAR) {
            return Sampler::Filter::FILTER_MIN_LINEAR_MAG_POINT_MIP_LINEAR;
        }
    } else if (magFilter == GL_LINEAR) {
        if (minFilter == GL_NEAREST) {
            return Sampler::Filter::FILTER_MIN_POINT_MAG_LINEAR;
        } else if (minFilter == GL_LINEAR) {
            return Sampler::Filter::FILTER_MIN_MAG_LINEAR;
        } else if (minFilter == GL_NEAREST_MIPMAP_NEAREST) {
            return Sampler::Filter::FILTER_MIN_POINT_MAG_LINEAR_MIP_POINT;
        } else if (minFilter == GL_NEAREST_MIPMAP_LINEAR) {
            return Sampler::Filter::FILTER_MIN_POINT_MAG_MIP_LINEAR;
        } else if (minFilter == GL_LINEAR_MIPMAP_NEAREST) {
            return Sampler::Filter::FILTER_MIN_MAG_LINEAR_MIP_POINT;
        } else if (minFilter == GL_LINEAR_MIPMAP_LINEAR) {
            return Sampler::Filter::FILTER_MIN_MAG_MIP_LINEAR;
        }
    }
...
static Sampler::WrapMode wrapModeFromGL(cgltf_int wrapMode) {
...
HFMTexture GLTFSerializer::getHFMTexture(const cgltf_texture *texture, cgltf_int texCoordSet) {
    ...
    if (sampler) {
        ...
        hfmTex.sampler = Sampler(samplerDesc);
    }
