...
Canvas {
    ...
    function refresh(invalidate) {
        ...
        dirty = true
        if (available && visible && source && source.visible && source.width > 0 && source.height > 0)
            captureTimer.restart()
    }
    function captureFrame() {
        if (busy || !dirty || !available || !source || !source.visible || !visible || source.width <= 0 || source.height <= 0) return
        dirty = false
        busy = true
        ...
        var started = item.grabToImage(function(result) {
            busy = false
            if (ticket === generation && item === source && visible) {
                clearCapture()
                snapshot = result
                captured = result.url
                loadImage(captured)
