...
using gpu::Type;
using gpu::Offset;
struct VKBuffer { VkBuffer buffer; };
struct Buffer {
...
struct VKBackend {
    ...
    struct { struct { std::array<Offset,2> _bufferStrides{}; std::bitset<2> _bufferStrideSet; } pipelineState; } _cache;
...
int main() {
    ...
    assert(vertexBinds==1 && !b.validateInputDraw(false,1,0,1,0));
    std::cout << "PASS input type, transfer isolation, null/range guards, instance offsets and valid draws\n";
}
'''
...
with tempfile.TemporaryDirectory(prefix='overte-input-') as temp:
    ...
    for platform in ['desktop', 'ios']:
        subprocess.run(['c++','-std=c++17','-O1',*(['-DQ_OS_IOS'] if platform == 'ios' else []),str(cpp),'-o',str(binary)],check=True,timeout=40)
