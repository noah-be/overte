...
TOOL = ROOT / "ios" / "ci" / "runner-telemetry.py"


def load_tool():
    spec = importlib.util.spec_from_file_location("ios_runner_telemetry", TOOL)
...
class RunnerTelemetryTest(unittest.TestCase):
    ...
    def test_emitter_appends_flushes_and_contains_no_sensitive_context(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(json.loads(lines[1])["ios_runner_telemetry"], "report")
    ...
    def test_supervised_output_is_forwarded_and_privately_preserved(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            output_log = root / "raw" / "phase.log"
            completed, records = self.invoke_phase(
                root,
                "print('compiler-output-marker', flush=True)",
                extra=["--output-log", str(output_log)],
            )
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertIn("compiler-output-marker", completed.stdout)
            self.assertEqual(output_log.read_text().strip(), "compiler-output-marker")
            self.assertEqual(output_log.stat().st_mode & 0o777, 0o600)
    ...
    def test_active_cpu_prevents_false_inactivity_abort_and_heartbeats(self):
        with tempfile.TemporaryDirectory() as directory:
            result, records = self.invoke_phase(
                Path(directory),
                # A complete iOS sample invokes ps, vm_stat, sysctl and
            ...
            heartbeats = [row for row in records
                          if row["ios_runner_telemetry"] == "heartbeat"]
    ...
    def test_live_output_prevents_false_inactivity_abort(self):
        with tempfile.TemporaryDirectory() as directory:
            result, records = self.invoke_phase(
                ...
                # A six-by-70ms window was too narrow on loaded iOS hosts:
    ...
    def test_active_phase_hits_controlled_wall_limit_before_outer_ci_timeout(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            # Keep the wall-limit test hermetic on iOS.  The system `sample`
            ...
            self.assertTrue(any(row["ios_runner_telemetry"] == "timed_out"
                                for row in records))
    ...
    def test_inactive_phase_is_killed_after_private_sanitized_diagnostics(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result.returncode, 124, result.stdout + result.stderr)
            self.assertTrue(any(row["ios_runner_telemetry"] == "stalled"
                                for row in records))
