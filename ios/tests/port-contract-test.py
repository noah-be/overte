...
def test_ci_contract() -> None:
    ...
    require_text(workflow, r'\[\[ "\$EVENT_NAME" == push \]\]', "push runs must retain iPhone and iPad coverage")
    require_text(workflow, r'elif \[\[ "\$EVENT_NAME" == pull_request \]\]', "manual runs must keep the single-iPhone default")
    require_text(workflow, r'families="iphone ipad"', "push and touch-relevant pull requests must cover iPhone and iPad")
    ...
    require_text(integrated, r"Restore durable Qt host artifact after cache eviction[\s\S]*?qt-checkpoint-artifact\.py restore[\s\S]*?qt_host_artifact_prefix", "the consumer must survive host-cache eviction through the validated artifact")
