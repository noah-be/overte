...
resume_method = application_ui_source.index(
    "void Application::resumeAfterLoginDialogActionTaken()", queued_resume
)
assert safe_resume < queued_resume < resume_method
ios_resume_boundary = application_ui_source[safe_resume:resume_method]
assert "if (_noLoginSuggestion || _resumeAfterLoginDialogActionTaken_WasPostponed)" in ios_resume_boundary
...
ios_mobile_ui = application_ui_source.index(
    "#if defined(Q_OS_IOS)\n"
    "    // The desktop toolbar is optional",
    resume_method,
)
...
ios_mobile_ui_end = application_ui_source.index("#else", ios_mobile_ui)
assert ios_mobile_ui < ios_mobile_ui_end < toolbar_lookup < toolbar_guard < toolbar_write
assert "getReticleInterface()->setVisible(true);" in application_ui_source[
    ios_mobile_ui:ios_mobile_ui_end
]
