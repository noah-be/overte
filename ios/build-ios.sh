...
The --e2e-test-build option is opt-in, device-only, and requires an explicit
Full Client bundle identifier ending in .e2e.
EOF
}
...
if ((e2e_test_build)); then
    [[ "$command_name" == "configure" || "$command_name" == "package-client" ]] \
        || fail "--e2e-test-build is only valid with configure or package-client"
    [[ "$platform" == "device" ]] \
        || fail "--e2e-test-build requires --platform device"
    ((bundle_id_explicit)) \
        || fail "--e2e-test-build requires an explicit --bundle-id ending in .e2e"
    [[ "$bundle_id" =~ [.]e2e$ ]] \
        || fail "the E2E Full Client bundle identifier must end in .e2e"
    if [[ "$command_name" == "configure" ]]; then
        ((client_graph)) || fail "--e2e-test-build requires --client-graph"
    fi
fi
...
run_package_client() {
    ...
    local e2e_contract_state="disabled"
    ((e2e_test_build)) && e2e_contract_state="enabled"
    python3 "$script_dir/tools/verify-e2e-test-build.py" \
        "$app_path/Info.plist" --expected "$e2e_contract_state" \
        || fail "integrated client E2E test-build contract failed validation"
...
PY
    ...
    python3 - "$manifest" "$latest_json" "$latest_text" "$archive" "$archive_sha" \
        ...
        "$symbols_archive" "$symbols_sha" "$app_uuid" "$e2e_test_build" <<'PY'
import json
import pathlib
import sys

manifest_name, latest_name, latest_text_name, archive_name, digest, revision, configuration, platform, signed, sequence, xcode, sdk, provisioned, application_identifier, get_task_allow, symbols_name, symbols_digest, binary_uuid, e2e_test_build = sys.argv[1:]
