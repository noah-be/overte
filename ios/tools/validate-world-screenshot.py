...
def validate(path: Path, scenario: str, destination: str) -> dict[str, object]:
    ...
    # A textured star skybox is valid presentation evidence, but is not proof
    # that world content is visible. Real geometry/particles introduce broad
    # spatial contrast even when the authored world is intentionally blue.
    if len(buckets) < 24 or deviation < 12.0:
