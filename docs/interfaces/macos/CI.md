...
and compiler cache are snapshotted again. The completed libnode generation is
also chunked, uploaded, downloaded, and manifest-verified as a 14-day durable
artifact independent of the repository-wide cache LRU. A later run prefers the
complete durable graph, then this libnode boundary, before rebuilding V8.
Restoring only an older Qt-stage cache no longer suppresses the newer libnode
save: exact full-cache hits and already verified durable restores are the only
conditions that skip compaction. Before the libnode Conan snapshot,
