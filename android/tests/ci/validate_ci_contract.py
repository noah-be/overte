...
REQUIRED_TIER_COMMANDS = {
    ...
    "tests/run-tests.sh mutation-extended",
    "tests/run-tests.sh stability",
...
REQUIRED_JUNIT_PATHS = {
    ...
    "android/tests/robolectric/build/test-results/test/",
...
def validate_periodic_jobs(workflow: str) -> list[str]:
    errors = []
    for job, command in (("stability", "tests/run-tests.sh stability"),
                         ("endurance", "tests/run-tests.sh endurance")):
    ...
    if endurance:
        ...
        if "tests/qml/run-qml-tests.sh" in body:
...
def main() -> int:
    ...
    parser.add_argument("--wrapper", type=Path, default=root / "android/gradle/wrapper/gradle-wrapper.properties")
    parser.add_argument("--robolectric-build", type=Path, default=root / "android/tests/robolectric/build.gradle")
    parser.add_argument("--robolectric-lock", type=Path, default=root / "android/tests/robolectric/gradle.lockfile")
