...
VALID_WORKFLOW = VALID_ACTION + """
    ...
    - name: next
      run: tests/run-tests.sh fast
    - run: tests/run-tests.sh contracts
    - run: tests/run-tests.sh regression
    - run: tests/run-tests.sh mutation
    - run: tests/run-tests.sh mutation-extended
    - run: tests/run-tests.sh stability
    - run: tests/run-tests.sh endurance
      path: |
...
class CiContractTest(unittest.TestCase):
    ...
    def test_rejects_missing_tier_command(self):
        errors = contract.validate_workflow(
            VALID_WORKFLOW.replace("run: tests/run-tests.sh contracts", "run: tests/run-tests.sh typo"))
