...
RUNNER = ROOT / "tests/suite/run.py"


class EnduranceInfrastructureTest(unittest.TestCase):
    def test_cycle_counts_fail_closed(self):
        cases = [
            (ROOT / "tests/javascript/run-lifecycle-endurance.sh", "OVERTE_JS_ENDURANCE_CYCLES"),
            (ROOT / "tests/native/run-endurance-tests.sh", "OVERTE_NATIVE_ENDURANCE_CYCLES"),
        ]
