...
JAVA_PRODUCTION = {
    ...
    "pending": ROOT / "apps/phoneInterface/src/main/java/org/overte/phone/PhonePendingUrlPolicy.java",
    "asset": ROOT / "libraries/qt/src/main/java/io/highfidelity/utils/SafeAssetPath.java",
    "extractor": ROOT / "libraries/qt/src/main/java/io/highfidelity/utils/AssetCacheExtractor.java",
}
...
HANDOFF = ROOT / "apps/phoneInterface/src/PhonePendingHandoff.h"
LOGIN_STATE = ROOT.parent / "interface/src/ui/PhoneLoginState.h"
NATIVE_TESTS = {
    "graphics": sorted((ROOT / "tests/native").glob("phone_graphics_*_test.cpp")),
    "handoff": sorted((ROOT / "tests/native").glob("phone_pending_handoff_*_test.cpp")),
    "login": sorted((ROOT / "tests/native").glob("phone_login_state_*_test.cpp")),
}
JAVASCRIPT_TESTS = {
    SCRIPTS / "+android_phoneInterface/mobileTabletApps.js": ROOT / "tests/javascript/test/mobile-tablet-apps.production.test.js",
    SCRIPTS / "+android_phoneInterface/mobileActionBar.js": ROOT / "tests/javascript/test/mobile-action-bar.production.test.js",
    SCRIPTS / "quickGoto.js": ROOT / "tests/javascript/test/quick-goto.production.test.js",
    SCRIPTS / "places/places.js": ROOT / "tests/javascript/test/places.production.test.js",
    SCRIPTS / "places/portal.js": ROOT / "tests/javascript/test/portal.production.test.js",
}
...
def native_run(work: Path, family: str, mutant: Mutant | None) -> tuple[str, str]:
    ...
    for test in NATIVE_TESTS[family]:
        ...
        result = command([
            ...
            "-I", str(ROOT / "tests/native/support"), str(test), "-o", str(binary),
        ], work)
        if result.returncode:
...
def javascript_run(work: Path, mutant: Mutant | None) -> tuple[str, str]:
    ...
    for test in tests:
        result = command(["node", "--test", str(test)], ROOT / "tests/javascript", env=environment)
        if result.returncode:
            output = result.stdout + result.stderr
