...
JAVA = (
    ROOT
    / "android/vr/pico/apps/picoInterface/src/main/java/org/overte/pico/AndroidAudioInput.java"
...
class PicoAudioTransportTests(unittest.TestCase):
    ...
    def test_capture_thread_startup_rolls_back_recorder_state(self):
        ...
        catch = body.index('RedactingDiagnostics.e(Event.REDACTED)', thread_start)
    ...
    def test_audio_stop_and_release_driver_errors_are_contained(self):
        ...
        stop_catch = stop.index("catch (RuntimeException error)", stop_call)
        join = stop.index("oldThread.join(1000)", stop_catch)
        release_call = stop.index("oldRecorder.release()", join)
        release_catch = stop.index("catch (RuntimeException error)", release_call)
        ...
        self.assertIn('SHUTDOWN.run(() ->', stop)
        self.assertIn('PicoAudioLifecycle.stopCompleted(stopped)', stop)
        self.assertIn('if (oldThread.isAlive()) clean = false', stop)
