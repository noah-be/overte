...
MANIFEST = ROOT / "android/vr/pico/apps/picoInterface/src/main/AndroidManifest.xml"
...
class AndroidEntrypointsTest(unittest.TestCase):
    ...
    def test_restart_arguments_are_private_and_not_logged(self):
        ...
        self.assertIn("store.read(SLOT)", storage)
        self.assertIn("store.remove(SLOT)", storage)
        self.assertLess(storage.index("store.remove(SLOT)"), storage.index("new String(bytes"))
        self.assertNotIn(".putString(", storage)
        self.assertIn("static synchronized boolean clear(Context context)", storage)
    ...
    def test_qt_activity_releases_static_android_resources(self):
        ...
        self.assertIn('runShutdownStep("microphone", () -> AndroidAudioInput.setForeground(false))', activity)
        self.assertIn('final boolean ownsLifecycle = INSTANCE.current() == this', activity)
        self.assertIn('if (ownsLifecycle)', activity)
        ...
        audio_cleanup = activity.index("AndroidAudioInput.setForeground(false)", destroy)
        ...
        self.assertIn('runShutdownStep("microphone", () -> AndroidAudioInput.setForeground(false))', destroy_body)
