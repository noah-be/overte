...
profile="$script_dir/conan/profiles/phone-nonqt-arm64-16k"
output_dir="$script_dir/conan/phone-nonqt-16k-debug"
...
"$conan_bin" export "$script_dir/conan/recipes/libnode"
"$conan_bin" export "$script_dir/conan/recipes/onetbb-local" --version=2021.10.0

echo "Rebuilding non-Qt shared dependencies with 16 KiB ELF LOAD alignment"
"$conan_bin" install "$script_dir/conan/conanfile-pico.py" \
    -of "$output_dir" \
    -pr:h "$profile" \
    -pr:b default \
    --no-remote \
