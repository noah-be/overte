...
from conan.tools.files import apply_conandata_patches, export_conandata_patches, copy, get, load, replace_in_file, rmdir, rm
...
class OneTBBConan(ConanFile):
    ...
    def _patch_android_linker_script(self):
        if self.settings.os == "Android":
            replace_in_file(
                self,
                os.path.join(self.source_folder, "cmake", "compilers", "Clang.cmake"),
                "else()\n"
    ...
    def build(self):
        ...
        self._patch_android_linker_script()
        cmake = CMake(self)
        cmake.configure()
        cmake.build()
