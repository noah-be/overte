...
resolve_android_file() {
    local relative_path="$1"

    case "$relative_path" in
        build-phone.sh) relative_path='phone/build.sh' ;;
        build-phone.gradle) relative_path='phone/build.gradle' ;;
        settings-phone.gradle) relative_path='phone/settings.gradle' ;;
        phone-prebuilt-16k-deps.sh|phone-device-lock.sh|prepare-phone-16k-conan-deps.sh|phone-build-resource-guard.sh|finalize-phone-16k-deps.sh)
            relative_path="phone/$relative_path"
            ;;
        tests/*) relative_path="phone/$relative_path" ;;
        docs/*) relative_path="phone/$relative_path" ;;
        conan/*) relative_path="common/$relative_path" ;;
        device-lock-core.sh) relative_path='common/scripts/device-lock-core.sh' ;;
    esac

    printf '%s/%s\n' "$android_root" "$relative_path"
}
...
require_file() {
    local relative_path="$1"
    local file_path
    file_path="$(resolve_android_file "$relative_path")"
    if [[ -f "$file_path" ]]; then
...
require_text() {
    ...
    file_path="$(resolve_android_file "$relative_path")"
    if [[ -f "$file_path" ]] &&
            grep -Eq -- "$pattern" "$file_path"; then
        pass "$description"
    else
        fail "$description"
    fi
...
reject_text() {
    ...
    file_path="$(resolve_android_file "$relative_path")"
    if [[ -f "$file_path" ]] &&
            ! grep -Eiq -- "$pattern" "$file_path"; then
        pass "$description"
    else
        fail "$description"
    fi
...
manifest='phone/apps/phoneInterface/src/main/AndroidManifest.xml'
gradle='phone/apps/phoneInterface/build.gradle'
cmake='phone/apps/phoneInterface/CMakeLists.txt'
permissions_activity='phone/apps/phoneInterface/src/main/java/org/overte/phone/PermissionsActivity.java'
interface_activity='phone/apps/phoneInterface/src/main/java/org/overte/phone/PhoneInterfaceActivity.java'
pending_url_policy='phone/apps/phoneInterface/src/main/java/org/overte/phone/PhonePendingUrlPolicy.java'
deep_link='phone/apps/phoneInterface/src/main/java/org/overte/phone/PhoneDeepLink.java'
...
require_text common/libraries/qt/src/main/java/io/highfidelity/utils/SafeAssetPath.java \
    'getCanonicalFile\(\)' \
...
require_text settings-phone.gradle \
    ...
    'phoneInterface resolves to phone/apps/phoneInterface'
require_text settings-phone.gradle \
...
if awk '
        ...
        in_task && /^}/ { exit }
        END { exit !found }
    ' "$(resolve_android_file "$gradle")"; then
...
require_text phone/apps/phoneInterface/src/main/java/org/qtproject/qt5/android/QtLayout.java \
...
reject_text 'phone/apps/phoneInterface/src/main/res/values/qt_dependencies.xml' \
...
require_text phone/apps/phoneInterface/build.gradle \
