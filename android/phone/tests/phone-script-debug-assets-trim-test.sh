...
gradle_file="$android_dir/phone/apps/phoneInterface/build.gradle"
...
apk="$android_dir/phone/apps/phoneInterface/build/outputs/apk/debug/phoneInterface-debug.apk"
if [[ -f "$apk" ]]; then
    if unzip -Z1 "$apk" | awk '
