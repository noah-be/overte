...
require_text "$android_dir/phone/docs/BUILD.md" 'phone-emulator-test\.sh all' \
    'emulator workflow is not documented'
require_text "$android_dir/phone/apps/phoneInterface/CMakeLists.txt" \
...
EOF
cat >"$fixture_android/common/gradlew" <<'EOF'
#!/usr/bin/env bash
count=0
...
env "${common_environment[@]}" PHONE_EMULATOR_TEST_CLASS="$test_class" \
    PHONE_EMULATOR_TEST_REPETITIONS=3 "$fixture_android/phone/phone-emulator-test.sh" test \
    >"$fixture/repeat.out"
...
env "${common_environment[@]}" FAKE_GRADLE_FAIL_ATTEMPT=2 \
    ...
    "$fixture_android/phone/phone-emulator-test.sh" test >"$fixture/failure.out" 2>&1
failure_status=$?
set -e
...
diagnostic_dir="$(find "$fixture_android/phone/build/phone-emulator/diagnostics" \
...
if PHONE_EMULATOR_TEST_REPETITIONS=26 "$fixture_android/phone/phone-emulator-test.sh" test \
...
if PHONE_EMULATOR_TEST_REPETITIONS=2 "$fixture_android/phone/phone-emulator-test.sh" test \
...
env "${common_environment[@]}" "$fixture_android/phone/phone-emulator-test.sh" doctor \
...
env "${common_environment[@]}" FAKE_GRADLE_MARKER="$first_marker" \
    ...
    "$fixture_android/phone/phone-emulator-test.sh" test >"$fixture/first-run.out" 2>&1 &
first_pid=$!
...
env "${common_environment[@]}" FAKE_GRADLE_MARKER="$second_marker" \
    "$fixture_android/phone/phone-emulator-test.sh" test >"$fixture/second-run.out" 2>&1 &
second_pid=$!
sleep 0.1
[[ ! -f "$second_marker" ]] || {
