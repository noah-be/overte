...
# Compile the actual diagnostic call with the production non-Phone header.
# This catches an unguarded call to the Phone-only predicate even though the
# PHONE_LOADING macro itself correctly disappears on other platforms.
repo = Path(__file__).resolve().parents[4]
diagnostic_start = source.index('\n', source.index('hfmModel = FBXSerializer().read(')) + 1
diagnostic_end = source.index('                PHONE_LOADING("phase=animation_parsed', diagnostic_start)
diagnostic = source[diagnostic_start:diagnostic_end]
non_phone = '#include <PhoneLoadingDiagnostics.h>\nvoid parse() {\n' + diagnostic + '\n}\n'
with tempfile.TemporaryDirectory(prefix="animation-non-phone-") as temporary:
    cpp = Path(temporary) / "test.cpp"
    cpp.write_text(non_phone)
    subprocess.run(["c++", "-std=c++17", "-Wall", "-Wextra", "-Werror", "-fsyntax-only",
                    "-I", str(repo / "libraries/shared/src"), str(cpp)], check=True)
...
print("PASS: production animation diagnostic call compiles without Phone-only symbols on other platforms.")
