...
from conan.tools.files import chdir, copy, load, replace_in_file, rm, rmdir, save, export_conandata_patches, apply_conandata_patches
...
class QtConan(ConanFile):
    ...
    def validate(self):
        ...
        unavailable = sorted(
            module
            for module in self._submodules
            if module not in self._composed_modules and self.options.get_safe(module)
        )
        if unavailable:
            raise ConanInvalidConfiguration(
                f"modules are absent from the locked source composition: {unavailable}"
            )
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_libjpeg", False) and not self.options.multiconfiguration:
            if self.options.with_libjpeg == "libjpeg-turbo":
                self.requires("libjpeg-turbo/[>=3.0.4 <3.1]")
            else:
    ...
    def _verify_composed_source(self, root):
        composition_path = root / "COMPOSITION.json"
        tree_lock_path = root / "TREE_LOCK.tsv"
        try:
            composition = json.loads(composition_path.read_text(encoding="utf-8"))
        except (OSError, ValueError) as error:
            raise ConanInvalidConfiguration(f"invalid Qt composition evidence: {error}")
        for key, expected in self._composition_contract.items():
            if composition.get(key) != expected:
                raise ConanInvalidConfiguration(f"Qt composition {key} is not locked")
        if self._sha256(tree_lock_path) != composition.get("tree_lock_sha256"):
            raise ConanInvalidConfiguration("Qt tree-lock digest mismatch")

        qt_root = root / "qt5"
        expected_rows = {}
        try:
            rows = tree_lock_path.read_text(encoding="utf-8").splitlines()
        except OSError as error:
            raise ConanInvalidConfiguration(f"unreadable Qt tree lock: {error}")
        if not rows or rows[0] != "path\tmode\tsha256":
            raise ConanInvalidConfiguration("invalid Qt tree-lock header")
        for row in rows[1:]:
            fields = row.split("\t")
            if len(fields) != 3:
                raise ConanInvalidConfiguration("invalid Qt tree-lock row")
            relative, mode, digest = fields
            path = PurePosixPath(relative)
            if path.is_absolute() or ".." in path.parts or not path.parts:
                raise ConanInvalidConfiguration("unsafe Qt tree-lock path")
            if relative in expected_rows or relative != path.as_posix():
                raise ConanInvalidConfiguration("duplicate or noncanonical Qt tree-lock path")
            if mode not in ("100644", "100755", "120000"):
                raise ConanInvalidConfiguration("invalid Qt tree-lock mode")
            if len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
                raise ConanInvalidConfiguration("invalid Qt tree-lock SHA-256")
            expected_rows[relative] = (mode, digest)
        if list(expected_rows) != sorted(expected_rows, key=PurePosixPath):
            raise ConanInvalidConfiguration("unsorted Qt tree lock")
        if len(expected_rows) != composition.get("tree_entries"):
            raise ConanInvalidConfiguration("Qt tree-lock entry count mismatch")

        actual_rows = {}
        for path in sorted(qt_root.rglob("*")):
            relative = path.relative_to(qt_root).as_posix()
            if path.is_symlink():
                mode = "120000"
                digest = hashlib.sha256(
                ...
                continue
            actual_rows[relative] = (mode, digest)
    ...
    def source(self):
        ...
        if not composed or not os.path.isabs(composed):
            raise ConanInvalidConfiguration(
                "OVERTE_QT_COMPOSED_SOURCE must name an explicit absolute path"
            )
        composed_root = Path(composed).resolve(strict=True)
        verified_qt_root = self._verify_composed_source(composed_root)
        destination = Path(self.source_folder) / "qt5"
        if destination.exists():
            raise ConanInvalidConfiguration("Qt source destination already exists")
        shutil.copytree(verified_qt_root, destination, symlinks=True)
    ...
    def package_info(self):
        ...
        if self.options.qtmultimedia:
            ...
            if self.settings.os == "Windows":
                ...
                _create_plugin("QWasapiPlugin", "qtaudio_wasapi", "audio", [])
            if self.settings.os == "Macos":
                _create_plugin("AudioCaptureServicePlugin", "qtmedia_audioengine", "mediaservice", [])
