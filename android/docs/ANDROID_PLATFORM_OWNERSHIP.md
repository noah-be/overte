...
`OffscreenGLCanvas.cpp` and shared Qt/TBB runtime overrides under `android/shared`;
...
`android/shared/vr/openxr/OpenXrDebugPolicy.h` classifies standard OpenXR debug
...
3. **Phone consumed Pico-owned paths at the assessed baseline.** A3 on
   ...
   common Qt/TBB runtime override directory under `android/shared`. Both product
