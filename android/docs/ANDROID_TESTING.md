...
```bash
tests/run-tests.sh prepared-host
```
...
run `tests/pico-host-regression-test.sh --no-build --build-dir <build>`. Both
...
The machine-readable [Phone test inventory](../tests/phone-test-inventory.json)
...
`tests/project-module-inventory.json`, which covers all seven Gradle modules:
...
Every CI job also runs `tests/reporting/generate_summary.py` under `if:
...
directly with `tests/ci/run-ci-contract-tests.sh`; it requires only Python's
...
`tests/run-tests.sh stability` is intentionally separate from pull-request
