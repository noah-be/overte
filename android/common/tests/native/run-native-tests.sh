...
case "$mode" in
    all)
        report_name=TEST-native.xml
        ctest_filter=()
        ;;
    android-vr)
        report_name=TEST-native-android-vr.xml
        ctest_filter=(-L '^android-vr$')
        ;;
    *)
        printf 'error: native test mode must be all or android-vr\n' >&2
        exit 2
        ;;
esac
...
overte_junit_prepare "$report_dir" "$report_name"
...
"$ctest_command" --test-dir "$build_dir" --output-on-failure \
    "${ctest_filter[@]}" --output-junit "$OVERTE_JUNIT_TEMP_REPORT"
status=$?
set -e
