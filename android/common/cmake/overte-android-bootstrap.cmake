# Shared Android bootstrap. Product callers provide prepared input locations.
file(MAKE_DIRECTORY "${CMAKE_BINARY_DIR}/cmake")
if(DEFINED ENV{OVERTE_FDROID_CONAN_DIR})
    # This source-built host graph takes precedence over legacy consumer inputs.
    if("$ENV{OVERTE_FDROID_CONAN_DIR}" STREQUAL "")
        message(FATAL_ERROR "OVERTE_FDROID_CONAN_DIR must not be empty")
    endif()
...
if(ANDROID)
    ...
    if(NOT TARGET Qt5::AndroidExtras)
        ...
        set_property(
            ...
            "${CMAKE_CURRENT_LIST_DIR}/android-compat"
        )
    endif()
    ...
    foreach(_android_qt_web_target WebEngineCore WebEngineWidgets)
        if(NOT TARGET Qt5::${_android_qt_web_target})
            add_library(Qt5::${_android_qt_web_target} INTERFACE IMPORTED)
            set_property(
                TARGET Qt5::${_android_qt_web_target}
                PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                "${CMAKE_CURRENT_LIST_DIR}/android-compat"
            )
        endif()
    ...
    if(NOT HIFI_ANDROID_CONAN_GENERATORS)
        message(FATAL_ERROR "HIFI_ANDROID_CONAN_GENERATORS must name the prepared target graph")
    endif()
