...
output_dir="$script_dir/conan/phone-16k-debug"
ready_sentinel="$script_dir/conan/phone-nonqt-16k-debug/.phone-16k-dependencies.ready"
profile="$script_dir/conan/profiles/phone-arm64-16k"
...
verify_alignment() {
    ...
    "$script_dir/tests/check-phone-elf-alignment.sh" "$package_dir" \
...
main() {
    ...
    "$conan_path" install "$script_dir/conan/conanfile-pico.py" \
        -of "$output_dir" \
        -pr:h "$profile" \
        -pr:b default \
        -nr \
