...
test("Phone preserves the selected emote when the chooser closes and restores on shutdown", () => {
    const { Script, avatarCalls, tablet } = start();
    const button = openApp(tablet);
    tablet.fromQml.emit({ method: "phoneEmote.play", name: "Crying" });
    tablet.screenChanged.emit("Home", "");

    // Phone hides the chooser so the avatar remains visible during playback.
    // Closing that chooser must not truncate the selected one-shot animation.
