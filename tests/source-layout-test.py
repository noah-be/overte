...
# Phone deliberately removed the old selectors in 5026252402 when adopting
# universal touch UI. Check their active replacements instead of restoring
# obsolete selectors merely to match the parent's retained mobile inventory.
PHONE_SHARED_MOBILE_REPLACEMENTS = {
    ...
    "interface/resources/qml/hifi/tablet/+android_phoneInterface/TabletGeneralPreferences.qml":
...
class SourceLayoutTests(unittest.TestCase):
    ...
    def test_branch_profile_matches_owned_sources(self):
        ...
        for path in policy["shared_mobile_files"]:
            selected = PHONE_SHARED_MOBILE_REPLACEMENTS.get(path, path)
            self.assertTrue((ROOT / selected).is_file(), "shared iOS/Phone dependency: " + selected)
