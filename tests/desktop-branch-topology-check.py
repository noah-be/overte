#!/usr/bin/env python3
"""Enforce parent-owned and target-owned paths on Desktop child branches."""
...
POLICY_PATH = "tests/desktop-branch-path-ownership.json"
TARGETS = ("linux-main", "windows-main")
...
def load_policy(repository: Path, main_revision: str) -> dict[str, Any]:
    result = git(repository, "show", f"{main_revision}:{POLICY_PATH}")
    if result.returncode != 0:
        raise ValueError(f"cannot load {POLICY_PATH} from main")
    try:
        policy = json.loads(_decode(result))
    except json.JSONDecodeError as exc:
        raise ValueError(f"invalid main ownership policy: {exc}") from exc
    ...
    if not isinstance(targets, dict) or set(targets) != set(TARGETS):
        raise ValueError("ownership policy must define exactly linux-main and windows-main")
...
def _changed_paths(
    ...
    result = git(
        ...
        main_revision, head, "--", *pathspecs,
    )
    if result.returncode != 0:
        return [], "cannot compare main-owned paths"
...
def validate(repository: Path, main_revision: str, head: str, target: str) -> list[str]:
    errors: list[str] = []
    if target not in TARGETS:
        return [f"unsupported desktop target: {target}"]

    base = git(repository, "rev-parse", "--verify", f"{main_revision}^{{commit}}")
    if base.returncode != 0:
        return ["cannot resolve current main commit"]
    ancestry = git(repository, "merge-base", "--is-ancestor", main_revision, head)
    if ancestry.returncode != 0:
        errors.append(f"{target} must contain the current main history")

    try:
        policy = load_policy(repository, main_revision)
    ...
    changes, diff_error = _changed_paths(
        repository, main_revision, head, [*parent_prefixes, *parent_files]
    )
    if diff_error:
    ...
    for status, path in changes:
        if path in parent_files or not _target_owns(path, rules):
            errors.append(f"main-owned path differs on {target}: {path}")
            continue
...
def main() -> int:
    ...
    errors = validate(
        args.repository.resolve(), args.main, args.head, args.target
    )
    if errors:
        for error in errors:
            print(f"error: {error}", file=sys.stderr)
        return 1
    print(f"PASS: Desktop path ownership is valid for {args.target}")
    return 0
