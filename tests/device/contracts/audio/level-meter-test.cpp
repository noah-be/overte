...
int main(int argc,char**argv){
 ...
 audio.insert("muted",true);auto muted=image();assert(muted!=full);
 view.setSource(QUrl::fromLocalFile(QString::fromLocal8Bit(argv[1])+"/LevelMeter.qml"));assert(view.status()==QQuickView::Ready);
