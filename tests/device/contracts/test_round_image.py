...
class RoundImage(unittest.TestCase):
 def test_actual_component_against_qt_image(self):
  ...
  with tempfile.TemporaryDirectory(prefix='round-image-') as d:
   ...
   if os.environ.get('OVERTE_ROUND_IMAGE_DROP_CACHE_SIZE'):
    mutant=d/'RoundImage.qml';mutant.write_text(qml.read_text().replace('drawing.loadImage(image.source, root.sourceSize)', 'drawing.loadImage(image.source)'));qml=mutant
