...
class AddressReentrancy(unittest.TestCase):
 def test_actual_reentrant_paths(self):
  ...
  entry=bool(os.environ.get('OVERTE_ADDRESS_INCLUDE_ENTRY'))
  ...
  with tempfile.TemporaryDirectory(prefix='address-reentrancy-') as d:
   ...
   binary=d/'test';subprocess.run(['c++','-std=c++17','-fPIC','-I',str(ROOT),'-I',str(d),str(cpp),'-o',str(binary),*(['-DOVERTE_ADDRESS_ENTRY_TEST'] if entry else []),*(['-DOVERTE_ADDRESS_QREGEXP_ADAPTER'] if entry and 'static const QRegExp PORT_REGEX' in source else []),*flags],check=True,timeout=30)
