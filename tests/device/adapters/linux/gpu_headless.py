...
class GpuHeadlessLifecycle:
    ...
    def __init__(self, target: dict, state_directory: Path) -> None:
        ...
        # The enclosing state directory is already unique per adapter target,
        # so adding another hash to the basename provides no collision safety.
        # Keeping this name short leaves room under Linux sockaddr_un.sun_path
        # when the adapter uses its normal /tmp state root.
    ...
    def _validate_runtime(value: object) -> dict:
        ...
        fingerprint_value = {
            **result,
            "sentinelSha256": _sha256(SENTINEL),
            "sessionGuardSha256": _sha256(SESSION_GUARD),
        }
    ...
    def _state_ready(self, state: dict) -> bool:
        ...
        # The root is reparented when the short-lived adapter operation exits;
        # immutable token, PGID, executable, argv and digest remain its anchor.
