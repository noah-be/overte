#!/usr/bin/env python3
"""Device-free process and native-input tests for the Linux adapter."""
...
ADAPTER = ROOT / "adapters/linux/adapter.py"
VERIFIER = ROOT / "verify_adapter.py"
SPEC = importlib.util.spec_from_file_location("linux_adapter", ADAPTER)
...
class LinuxAdapterTest(unittest.TestCase):
    ...
    def test_posix_cleanup_detects_child_after_group_leader_exits(self):
        ...
        try:
            self.assertFalse(DESKTOP_MODULE.LinuxAdapter.state_alive(state))
            self.assertTrue(DESKTOP_MODULE.LinuxAdapter.process_tree_alive(state))
            DESKTOP_MODULE.LinuxAdapter.terminate_process_tree(parent.pid, force=True)
            deadline = time.monotonic() + 5
            while (DESKTOP_MODULE.LinuxAdapter.process_tree_alive(state)
                   and time.monotonic() < deadline):
                time.sleep(0.05)
            self.assertFalse(DESKTOP_MODULE.LinuxAdapter.process_tree_alive(state))
        finally:
            try:
    ...
    def setUp(self):
        ...
        self.environment.update({
            "OVERTE_LINUX_TARGETS": str(self.config),
    ...
    def test_new_capabilities_require_probe_command_control_on_every_desktop_variant(self):
        ...
        self.assertTrue(controlled.issubset(
            DESKTOP_MODULE.LinuxAdapter.capabilities(linux)))
    ...
    def test_navigation_and_asset_payloads_use_the_running_probe_process(self):
        ...
        adapter.prepare_injected_probe("linux-alias")
    ...
    def test_injected_launch_uses_private_probe_copy_in_the_authoritative_process(self):
        ...
        while (not (self.root / "interface-calls.log").exists()
               and time.monotonic() < deadline):
            time.sleep(0.02)
    ...
    def test_control_operations_reject_missing_probe_invalid_arguments_and_process_switch(self):
        ...
        payload = json.loads(self.config.read_text(encoding="utf-8"))
        linux = next(item for item in payload["targets"] if item["platform"] == "linux")
        linux.pop("clientControl")
        linux["probe"] = {"kind": "host-file", "path": str(self.probe)}
        self.config.write_text(json.dumps(payload), encoding="utf-8")
        with patch.dict(os.environ, self.environment, clear=True):
            uncontrolled = DESKTOP_MODULE.LinuxAdapter()
        with patch.object(uncontrolled, "read_state", return_value=state), patch.object(
            ...
            uncontrolled.invoke("linux-alias", "navigation.enter-domain", {
                "url": "hifi://127.0.0.1:40102/",
            })
    ...
    def test_all_desktop_manifests_satisfy_protocol(self):
        for platform in ("linux",):
            result = subprocess.run([
                ...
                str(ROOT / "adapters/linux/adapter.json"),
                "--check-cleanup",
            ], text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
               env=self.environment, check=False)
            self.assertEqual(0, result.returncode, result.stdout)
    ...
    def test_linux_physical_target_can_opt_into_xcb_on_xwayland(self):
        ...
        with patch.dict(os.environ, {
            ...
            DESKTOP_MODULE.LinuxAdapter().discover()
