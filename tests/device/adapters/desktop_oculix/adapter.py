#!/usr/bin/env python3
"""Cross-platform desktop adapter with native, target-scoped input drivers."""
...
try:
    ...
    WAYLAND_LIBEI_AVAILABLE = True
except ModuleNotFoundError as error:  # R08 supplies the optional transport.
    if error.name != "adapters.desktop_oculix.wayland_libei_client":
        raise
    WaylandInputClient = None
    WaylandInputError = RuntimeError
    default_socket_path = None
    WAYLAND_LIBEI_AVAILABLE = False
...
class DesktopAdapter:
    def __init__(self, platform: str) -> None:
        ...
        self.adapter_id = f"desktop-oculix-{platform}"
    ...
    def require_interactive_host(self) -> None:
        ...
        physical = bool(physical_targets)
        actual = "windows" if os.name == "nt" else "macos" if sys.platform == "darwin" else "linux"
        if physical and actual != self.platform:
            fail("physical desktop target platform does not match the host")
        if physical and actual == "windows":
        ...
        if physical and self.platform == "linux":
            interactive_targets = [
            ...
            if interactive_targets:
                ...
                if session != "x11" and not xwayland:
                    fail("Linux OculiX targets require X11 or an explicit xcb/XWayland target")
                if not os.environ.get("DISPLAY"):
                    fail("Linux OculiX targets require DISPLAY")
    ...
    def load_targets(self) -> dict[str, dict]:
        config_value = os.environ.get("OVERTE_DESKTOP_TARGETS")
        if not config_value:
            fail("OVERTE_DESKTOP_TARGETS must name a private target configuration")
        ...
        for entry in entries:
            if not isinstance(entry, dict) or entry.get("platform") not in PLATFORMS:
            ...
            if entry["platform"] in {"macos", "windows"}:
                if not all(isinstance(entry.get(field), str) and entry[field]
                           for field in ("oculixJar", "oculixSha256")):
                    fail("macOS and Windows targets require an OculiX JAR and SHA-256")
                if not re.fullmatch(r"[0-9a-fA-F]{64}", entry["oculixSha256"]):
                    fail("desktop target OculiX SHA-256 must contain 64 hexadecimal digits")
            ...
            if entry.get("xwayland") and (entry["platform"] != "linux"
            ...
            if entry.get("isolatedX11"):
                if (entry["platform"] != "linux" or entry.get("xwayland")
                        or environment.get("QT_QPA_PLATFORM") != "xcb"
    ...
    def discover(self) -> list[dict]:
        ...
        return [{
            "selector": selector,
            "displayName": target.get("displayName", f"Overte {self.platform}"),
    ...
    def target_environment(self, target: dict, *, visual_driver: bool = False) -> dict[str, str]:
        ...
        if self.platform == "linux" and target.get("isolatedX11"):
            ...
            if visual_driver:
                # OculiX only needs X11 and filesystem access. With no session
                # bus address it cannot accidentally invoke the GNOME
                # ScreenCast/RemoteDesktop portal.
    ...
    def wayland_input_socket_path(self, target: dict) -> Path:
        if not WAYLAND_LIBEI_AVAILABLE:
            fail("Wayland/libei transport is unavailable")
    ...
    def ensure_wayland_input(self, selector: str, target: dict, *, authorize: bool) -> dict:
        if self.platform != "linux" or target.get("inputDriver") != "wayland-libei":
    ...
    def process_tree_alive(cls, state: dict) -> bool:
        """Return whether the launched process tree can still need cleanup."""
        if os.name == "nt":
            # Windows has no persistent process-group query equivalent. The
            # launch token still prevents a reused leader PID from being
            # mistaken for the process we created.
        ...
        try:
            ...
            # A group created by this account should remain signalable. Fail
            # closed so cleanup reports the unexpected ownership boundary.
            return True
    ...
    def launch(self, selector: str, target: dict) -> dict:
        ...
        with log.open("ab") as output:
            ...
            options: dict = {"cwd": working, "stdout": output, "stderr": subprocess.STDOUT,
                             "env": environment}
            if os.name == "nt":
    ...
    def oculix(self, target: dict, action: str, values: dict) -> None:
        ...
        if self.platform == "linux":
            xdotool = expanded_path(target["xdotoolExecutable"])
            if (not xdotool.is_file() or file_sha256(xdotool).lower()
                    != target["xdotoolSha256"].lower()):
                fail("configured xdotool executable failed its SHA-256 check")
            environment["PATH"] = str(xdotool.parent) + os.pathsep + environment.get("PATH", "")
        result = subprocess.run(
    ...
    def invoke(self, selector: str, operation: str, values: dict) -> dict:
        ...
        if operation == "artifact.screenshot":
            ...
            if self.platform == "linux":
                ...
                self.oculix(target, "screenshot", {"artifactDirectory": artifact_dir,
...
def main() -> int:
    args = cli()
    adapter = DesktopAdapter(args.platform)
