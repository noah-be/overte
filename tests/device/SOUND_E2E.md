...
The module requires all of these independent observations, in order:
...
5. The probe observes a non-null injector from `Audio.playSound()` and then its
...
Only the deterministic mock advertises `sound.play`. Android, Pico, Appium,
iOS, Linux, Windows, and macOS adapter implementations and profiles are
...
that work is outside this test-side change.

## Proof boundary

Passing proves that Interface requested the controlled bytes, its production
sound resource reached decoded/usable state through the WAV path, and an
