...
No Android, Pico, Appium, iOS, Linux, Windows, or macOS adapter advertises or
implements `asset.load`, and Jenkins does not schedule this suite for a real
target. Real-adapter activation and physical acceptance are deliberately still
pending.
...
only as a diagnostic; it is never treated as completion evidence. A future real
...
The general fixture server publishes `asset` connection metadata in its ready
file. `tests/device/jenkins/run_ci.py` can prepare that metadata for an explicit
`asset-smoke` invocation, but no Jenkinsfile stage enables it today.
