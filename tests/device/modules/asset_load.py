...
def required_environment(name: str) -> str:
    ...
    if not value:
        raise InfrastructureError(f"{name} is required")
    return value
...
def positive_integer_environment(name: str) -> int:
    ...
    if not value.isdigit() or int(value) <= 0:
        raise InfrastructureError(f"{name} must be a positive integer")
    return int(value)


def request_url(base_url: str, request_id: str) -> str:
    if not REQUEST_ID.fullmatch(request_id):
        raise InfrastructureError("controlled asset request ID is invalid")
    parsed = urlsplit(base_url)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc or parsed.fragment:
        raise InfrastructureError("OVERTE_E2E_ASSET_URL must be an absolute HTTP URL")
    query = parse_qsl(parsed.query, keep_blank_values=True)
    if any(name == "requestId" for name, _value in query):
        raise InfrastructureError("OVERTE_E2E_ASSET_URL must not contain requestId")
...
def read_telemetry(base_url: str, asset_id: str, request_id: str) -> dict:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise InfrastructureError(
            "OVERTE_E2E_ASSET_TELEMETRY_URL must be an absolute HTTP URL")
...
def validate_delivery(value: dict, *, asset_id: str, request_id: str,
    ...
    expected = {
        ...
        "path": urlsplit(asset_url).path, "status": 200, "contentType": content_type,
