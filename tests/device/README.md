...
deterministic sound described in [`SOUND_E2E.md`](SOUND_E2E.md). The
...
`flying`, `flyingEnabled`, camera orientation, tablet state, sound resource and
injector state, and build identity through Interface's existing test-script
result API. It records no audio samples. Product adapters own the exact launch
and result transport used to load it.
