...
class AppiumAdapterTest(unittest.TestCase):
    ...
    def test_points_and_configured_durations_are_bounded(self):
        for value in ([1.0, .5], [float("nan"), .5]):
            with self.subTest(value=value), self.assertRaises(RuntimeError):
                APPIUM.AppiumAdapter.validate_fractional_point(value, "point")
        for value in (0, 10.1, float("inf")):
            with self.subTest(value=value), self.assertRaises(RuntimeError):
                APPIUM.AppiumAdapter.validate_duration(value, "duration")

    def test_physical_session_fails_before_network_access(self):
        with mock.patch.object(APPIUM, "WebDriver") as webdriver:
            with self.assertRaisesRegex(RuntimeError, "physical Android targets require"):
                self.load("android", [target("android", physical=True)])
    ...
    def test_every_advertised_invoke_path_performs_its_w3c_operation(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.dict(os.environ, environment), mock.patch.object(
                    ...
                    return_value=(client, "shared-session", {"sessionId": "shared-session"})):
                operations = {
                    "app.foreground": {},
                    ...
                    "app.launch": {},
                    "artifact.screenshot": {},
                    "input.look": {"horizontal": .4, "vertical": 0},
        ...
        self.assertEqual(set(adapter.advertised_capabilities(configured)), invoked)
    ...
    def test_session_state_is_reused_and_cleanup_is_idempotent(self):
        ...
        with tempfile.TemporaryDirectory() as state_root, mock.patch.dict(
            ...
            self.assertEqual("shared-session", first[1])
    ...
    def test_cleanup_failure_retains_private_session_state(self):
        ...
        with tempfile.TemporaryDirectory() as state_root, mock.patch.dict(
            ...
            adapter.save_session("shared-android", {"sessionId": "shared-session"})
            state_path = adapter.state_path("shared-android")
            with mock.patch.object(APPIUM, "WebDriver", return_value=FailingDeleteClient()):
                with self.assertRaisesRegex(RuntimeError, "private retry state was retained"):
    ...
    @unittest.skipIf(os.name == "nt", "POSIX state ownership and modes")
    def test_state_directories_are_private_and_reject_symlink_components(self):
        adapter = self.load("android", [target("android")])
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "state"
            root.mkdir(mode=0o755)
            with mock.patch.dict(os.environ, {"OVERTE_DEVICE_STATE_ROOT": str(root)}):
                state = adapter.state_path("shared-android")
                ...
                elsewhere.mkdir()
                state.parent.symlink_to(elsewhere, target_is_directory=True)
                with self.assertRaisesRegex(RuntimeError, "account-owned directory"):
                    adapter.state_path("shared-android")
            linked = Path(temporary) / "linked"
            linked.symlink_to(root, target_is_directory=True)
            with mock.patch.dict(os.environ, {"OVERTE_DEVICE_STATE_ROOT": str(linked / "nested")}):
                with self.assertRaisesRegex(RuntimeError, "symbolic links"):
                    adapter.state_path("shared-android")
