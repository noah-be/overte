...
class AssetLoadTest(unittest.TestCase):
    ...
    def run_asset_suite(self, mock_flag: str | None = None):
        ...
        result = subprocess.run([
            ...
            "--catalog", str(ROOT / "catalog.json"), "--suite", "asset-smoke",
    ...
    def test_mock_runs_complete_asset_smoke_suite(self):
    ...
    def assert_asset_failure(self, flag: str, expected: str) -> None:
        ...
        try:
            ...
            self.assertEqual("failed", json.loads(
                (root / "results/summary.json").read_text())["status"])
        finally:
            temporary.cleanup()
    ...
    def test_real_adapters_do_not_advertise_or_implement_asset_load(self):
        for relative in (
                "adapters/android/adapter.py", "adapters/appium/adapter.py",
                "adapters/desktop_oculix/adapter.py"):
            with self.subTest(adapter=relative):
                self.assertNotIn('"asset.load"', (ROOT / relative).read_text(encoding="utf-8"))
