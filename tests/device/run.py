...
def validate_manifest(path: Path) -> dict:
    ...
    if not isinstance(value.get("id"), str) or not value["id"]:
        fail("adapter manifest requires an id")
    load_command(path)
    return value
...
def load_modules(path: Path, suite: str) -> list[dict]:
    ...
    for module in value["modules"]:
        ...
        if not isinstance(identifier, str) or not identifier or identifier in seen:
            fail("module ids must be unique non-empty strings")
        seen.add(identifier)
        ...
        if not isinstance(requires, list) or not all(
            ...
            fail(f"module {identifier} has invalid capabilities")
...
def run_module(module: dict, catalog: Path, environment: dict[str, str],
               artifact_dir: Path, selector: str) -> dict:
    ...
    status = "skipped" if process.returncode == 77 else "passed" if process.returncode == 0 else "failed"
    if timed_out:
        status = "failed"
...
def write_junit(results: list[dict], path: Path, suite: str) -> None:
    root = ET.Element("testsuite", name=f"device-{suite}", tests=str(len(results)),
                      ...
                      errors="0", time=f"{sum(r['durationSeconds'] for r in results):.3f}")
