...
macro(AUTOSCRIBE_PLATFORM_SHADER)
    ...
    # The phone renderer consumes only the mono GLES source and reflection at
    # runtime. Keep the other Android and desktop shader bundles unchanged.
    if (NOT (HIFI_ANDROID AND HIFI_ANDROID_APP STREQUAL "phoneInterface") OR
            (AUTOSCRIBE_DIALECT STREQUAL "310es" AND AUTOSCRIBE_VARIANT STREQUAL "mono"))
    ...
    if (NOT (HIFI_ANDROID AND HIFI_ANDROID_APP STREQUAL "phoneInterface"))
    ...
    list(APPEND SHADER_GEN_LINE ${TEMP_PATH})
    set(AUTOSCRIBE_TARGET_DEFINES "${DEFINES}")
    if (HIFI_ANDROID AND HIFI_ANDROID_APP STREQUAL "phoneInterface")
        if (AUTOSCRIBE_TARGET_DEFINES STREQUAL "")
            set(AUTOSCRIBE_TARGET_DEFINES "phone")
        else()
            string(APPEND AUTOSCRIBE_TARGET_DEFINES "_phone")
        endif()
    endif()
    if (NOT("${AUTOSCRIBE_TARGET_DEFINES}" STREQUAL ""))
        list(APPEND SHADER_GEN_LINE "defines:${AUTOSCRIBE_TARGET_DEFINES}")
    endif()
        ...
        foreach(PROGRAM_FILE ${SHADER_PROGRAM_FILES})
            ...
            foreach(DEFINES ${DEFINES_LIST})
                ...
                if (NOT("${PROGRAM_DEFINES}" STREQUAL ""))
                    ...
                    set(DEFERRED_DEFINES "lightmap" "triplanar" "layers2" "layers3" "splatmap")
                    set(STARTUP_PROGRAM TRUE)
                    foreach(DEFERRED_DEFINE IN LISTS DEFERRED_DEFINES)
                        string(FIND "${PROGRAM_DEFINES}" "${DEFERRED_DEFINE}" INDEX)
                        if (NOT ${INDEX} EQUAL -1)
                            set(STARTUP_PROGRAM FALSE)
                            break()
                        endif()
                    endforeach()
